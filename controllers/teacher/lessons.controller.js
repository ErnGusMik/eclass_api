...
import { createHomework, getHomework, updateHomeworkData } from "../../models/homework.model.js";
import {
    getAllLessons,
    getLessonClass,
    getLessonsInDay,
...
const updateLesson = async (req, res, next) => {
    ...
    if (req.query.field == "notes") {
        ...
        if (!homework) {
            ...
            const updated = await updateHomeworkData(homework.id, req.body.content);
            if (updated) {
                res.sendStatus(204);
            } else {
                res.status(500).json({
...
const getAllTimes = async (req, res, next) => {
    ...
    if (!classTeacher) return res.status(403).json({
...
export { getDayLessons, updateLesson, getAllTimes };
